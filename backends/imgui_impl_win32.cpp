...
static short ImGui_ImplWin32_GetKeyState(int nVirtKey)
{
...
static bool ImGui_ImplWin32_InitEx(void* hwnd, short(*GetKeyStatePFn)(int), bool platform_has_own_dc)
{
    ...
    bd->GetKeyState = GetKeyStatePFn == nullptr ? &ImGui_ImplWin32_GetKeyState : GetKeyStatePFn;
...
IMGUI_IMPL_API bool     ImGui_ImplWin32_Init(void* hwnd, short(*GetKeyStatePFn)(int))
{
    return ImGui_ImplWin32_InitEx(hwnd, GetKeyStatePFn, false);
}

IMGUI_IMPL_API bool     ImGui_ImplWin32_InitForOpenGL(void* hwnd, short(*GetKeyStatePFn)(int))
{
    // OpenGL needs CS_OWNDC
    return ImGui_ImplWin32_InitEx(hwnd, GetKeyStatePFn, true);
}
...
static bool IsVkDown(ImGui_ImplWin32_Data* bd, int vk)
{
    return (bd->GetKeyState(vk) & 0x8000) != 0;
}
...
static void ImGui_ImplWin32_ProcessKeyEventsWorkarounds(ImGui_ImplWin32_Data* bd, ImGuiIO& io)
{
    // Left & right Shift keys: when both are pressed together, Windows tend to not generate the WM_KEYUP event for the first released one.
    if (ImGui::IsKeyDown(ImGuiKey_LeftShift) && !IsVkDown(bd, VK_LSHIFT))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftShift, false, VK_LSHIFT);
    if (ImGui::IsKeyDown(ImGuiKey_RightShift) && !IsVkDown(bd, VK_RSHIFT))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightShift, false, VK_RSHIFT);

    // Sometimes WM_KEYUP for Win key is not passed down to the app (e.g. for Win+V on some setups, according to GLFW).
    if (ImGui::IsKeyDown(ImGuiKey_LeftSuper) && !IsVkDown(bd, VK_LWIN))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftSuper, false, VK_LWIN);
    if (ImGui::IsKeyDown(ImGuiKey_RightSuper) && !IsVkDown(bd, VK_RWIN))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightSuper, false, VK_RWIN);
}

static void ImGui_ImplWin32_UpdateKeyModifiers(ImGui_ImplWin32_Data* bd, ImGuiIO& io)
{
    io.AddKeyEvent(ImGuiMod_Ctrl, IsVkDown(bd, VK_CONTROL));
    io.AddKeyEvent(ImGuiMod_Shift, IsVkDown(bd, VK_SHIFT));
    io.AddKeyEvent(ImGuiMod_Alt, IsVkDown(bd, VK_MENU));
    io.AddKeyEvent(ImGuiMod_Super, IsVkDown(bd, VK_LWIN) || IsVkDown(bd, VK_RWIN));
}

static void ImGui_ImplWin32_UpdateMouseData(ImGui_ImplWin32_Data* bd, ImGuiIO& io)
{
    IM_ASSERT(bd->hWnd != 0);
...
void    ImGui_ImplWin32_NewFrame()
{
    ...
    ImGui_ImplWin32_UpdateMouseData(bd, io);

    // Process workarounds for known Windows key handling issues
    ImGui_ImplWin32_ProcessKeyEventsWorkarounds(bd, io);
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
    ...
    case WM_SYSKEYUP:
    {
        ...
        {
            // Submit modifiers
            ImGui_ImplWin32_UpdateKeyModifiers(bd, io);
            ...
            if (vk == VK_SHIFT)
            {
                ...
                if (IsVkDown(bd, VK_LSHIFT) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftShift, is_key_down, VK_LSHIFT, scancode); }
                if (IsVkDown(bd, VK_RSHIFT) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightShift, is_key_down, VK_RSHIFT, scancode); }
            }
            else if (vk == VK_CONTROL)
            {
                if (IsVkDown(bd, VK_LCONTROL) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftCtrl, is_key_down, VK_LCONTROL, scancode); }
                if (IsVkDown(bd, VK_RCONTROL) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightCtrl, is_key_down, VK_RCONTROL, scancode); }
            }
            else if (vk == VK_MENU)
            {
                if (IsVkDown(bd, VK_LMENU) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftAlt, is_key_down, VK_LMENU, scancode); }
                if (IsVkDown(bd, VK_RMENU) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightAlt, is_key_down, VK_RMENU, scancode); }
            }
        }
        return 0;
    }
